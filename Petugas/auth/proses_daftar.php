...
if(isset($_POST['username']) && isset($_POST['no_telepon']) && isset($_POST['password'])) {
    ...
    $check_query = "SELECT * FROM tb_petugas WHERE username = ?";
    ...
    if(mysqli_num_rows($result_check) > 0){
        ...
        $query = "INSERT INTO tb_petugas (id_petugas, username, password, no_telepon) 
                  VALUES (NULL, ?, ?, ?)";
        ...
        if($result){
            header("location: ../Login petugas/login.php");
            exit();
        }
        else{
            header("location:../Login petugas/login.php?pesan=error");
            exit();
        }
    }
}    
